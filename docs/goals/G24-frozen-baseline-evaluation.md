...
2. Frozen-manifest binding and single-reviewer limitation:
   `test_g24_manifest_binding_and_limitation`.
3. Conservative status, contradiction, exclusion, malformed, and budget
   handling: `test_g24_conservative_outcomes`.
4. Offline replay, deterministic report digest, and no subprocess/network:
   `test_g24_replay_is_offline_and_byte_stable`.
5. Full locked suite, `uv lock --check`, documentation checks, and
   `git diff --check`.
